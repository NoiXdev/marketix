      ...
      <div className="px-8 py-8">
        ...
        {urls.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white py-16 dark:border-slate-700 dark:bg-slate-900">
            ...
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">{t('links.empty')}</p>
            <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">{t('links.empty_hint')}</p>
            <Link
              href={route('app.project.links.create', { project: project!.id })}
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                          <Link
                            href={route('app.project.links.show', { project: project!.id, url: url.id })}
                            title={t('links.actions.view_stats')}
                          ...
                          <button
                            onClick={() => toggle(url)}
                            title={url.status === 1 ? t('links.actions.deactivate') : t('links.actions.activate')}
