...
@section('body')
    ...
    <h2 style="font-size:14px;margin:20px 0 8px;color:#0f172a;">Recent clicks</h2>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="font-size:12px;">
        <tr>
            <th align="left" style="border-bottom:1px solid #e2e8f0;padding:4px 8px;color:#64748b;">When</th>
            <th align="left" style="border-bottom:1px solid #e2e8f0;padding:4px 8px;color:#64748b;">Country</th>
            <th align="left" style="border-bottom:1px solid #e2e8f0;padding:4px 8px;color:#64748b;">City</th>
            <th align="left" style="border-bottom:1px solid #e2e8f0;padding:4px 8px;color:#64748b;">Browser</th>
            <th align="left" style="border-bottom:1px solid #e2e8f0;padding:4px 8px;color:#64748b;">OS</th>
        </tr>
        @forelse ($recentClicks as $click)
            <tr>
                <td style="padding:4px 8px;border-bottom:1px solid #f1f5f9;">{{ $click['created_at'] }}</td>
                <td style="padding:4px 8px;border-bottom:1px solid #f1f5f9;">{{ $click['country'] ?? '—' }}</td>
                <td style="padding:4px 8px;border-bottom:1px solid #f1f5f9;">{{ $click['city'] ?? '—' }}</td>
                <td style="padding:4px 8px;border-bottom:1px solid #f1f5f9;">{{ $click['browser'] ?? '—' }}</td>
                <td style="padding:4px 8px;border-bottom:1px solid #f1f5f9;">{{ $click['os'] ?? '—' }}</td>
            </tr>
        @empty
            <tr><td colspan="5" style="padding:4px 8px;color:#94a3b8;">No data</td></tr>
        @endforelse
    </table>
@endsection
